/**
 * ph_address.js — Philippine Cascading Address Dropdowns
 * Order: Region → Province → City/Municipality
 */
...
const PH_DATA = {
  ...
  "Region IV-A - CALABARZON": {
    ...
    "Laguna": ["Alaminos","Bay","Biñan City","Cabuyao City","Calamba City","Calauan","Cavinti","Famy","Kalayaan","Liliw","Los Baños","Luisiana","Lumban","Mabitac","Magdalena","Majayjay","Mabitac","Nagcarlan","Paete","Pagsanjan","Pakil","Pangil","Pila","Rizal","San Pablo City","San Pedro City","Santa Cruz","Santa Maria","Santa Rosa City","Siniloan","Victoria"],
  ...
  "Region VI - Western Visayas": {
    ...
    "Capiz": ["Cuartero","Dao","Dumalag","Dumarao","Ivisan","Jamindan","Ma-ayon","Maayon","Mambusao","Panay","Panitian","Pilar","Pontevedra","President Roxas","Roxas City","Sapi-an","Sigma","Tapaz"],
  ...
  "Region VIII - Eastern Visayas": {
    ...
    "Samar": ["Almagro","Basey","Calbayog City","Calbiga","Catbalogan City","Daram","Gandara","Hinabangan","Jiabong","Marabut","Matuguinao","Motiong","Paranas","Pinabacdao","San Jorge","San Jose de Buan","San Sebastian","Santa Margarita","Santa Rita","Santo Niño","Tagapul-an","Talalora","Tarangnan","Villareal","Wagwag","Zumarraga"],
...
function initAddressDropdowns() {
  ...
  const provinceSel = document.getElementById('reg-province');
  const citySel     = document.getElementById('reg-city');
  if (!regionSel || !provinceSel || !citySel) return;

  // Populate regions
  Object.keys(PH_DATA).sort().forEach(region => {
  ...
  regionSel.addEventListener('change', function () {
    ...
    const provinces = PH_DATA[this.value];
    if (!provinces) return;
  ...
  provinceSel.addEventListener('change', function () {
    ...
    const cities    = PH_DATA[region]?.[this.value];
    if (!cities) return;
...
(function attachBarangayLoader() {
  const citySel     = document.getElementById('reg-city');
  const barangaySel = document.getElementById('reg-barangay');
  if (!citySel || !barangaySel) return; // safety — do nothing if elements missing
  ...
  citySel.addEventListener('change', async function () {
    ...
    barangaySel.style.opacity = '0.55';

    const cityName = this.value;
    if (!cityName) return;

    try {
      const res  = await fetch(`../admin/get_address.php?type=city_barangays&city_name=${encodeURIComponent(cityName)}`);
      const data = await res.json();

      if (data.success && data.barangays?.length) {
  ...
  // Also reset barangay when province or region changes
  document.getElementById('reg-province')?.addEventListener('change', function () {
    barangaySel.innerHTML = '<option value="">— Select Barangay —</option>';
    barangaySel.disabled  = true;
    barangaySel.style.opacity = '0.55';
  });
